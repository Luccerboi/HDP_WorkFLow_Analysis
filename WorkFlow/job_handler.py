...
def submit_and_wait(submission="vasp.sub"):
    ...
    get_jobid = subprocess.Popen(["awk", '{print $NF}'], stdin=submit.stdout, stdout=subprocess.PIPE, text=True)
    jobid, error = get_jobid.communicate()
    #print(f'Appointed jobID: {jobid}' )
    
    #check_state = subprocess.Popen(["sacct", "-j", jobid], stdout=subprocess.PIPE, text=False)
    #get_status = subprocess.Popen(["awk","-v","J=%s"%(jobid),  "/J +/ {print $6}"], stdin=check_state.stdout, stdout=subprocess.PIPE, text=True)
    done=False
    while done==False:
        ...
        check_state = subprocess.Popen(["sacct", "-j", jobid], stdout=subprocess.PIPE, text=False)
        get_status = subprocess.Popen(["awk",  "/%i +/ {print $6}"%(int(jobid)) ], stdin=check_state.stdout, stdout=subprocess.PIPE, text=True)
...
def check_availability():
    ...
    """
    p1 = subprocess.Popen(['sinfo'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    p2 = subprocess.Popen(['awk', '/ccp2/&&/idle/ {print $1 "   " $4}'], stdin=p1.stdout, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
