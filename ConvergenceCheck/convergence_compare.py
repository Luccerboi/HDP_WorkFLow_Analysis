...
Script used to make a side to side comparison between Density of States calculated with EDIFF=1e-4 and with EDIFF=1e-6.
Uses the plot_dos from `HDP_PlotslyPlot.py` (needs to be added to PYTHON_PATH or copied into directory).
Reparsed the DOS data to conform to the standard. 
"""
...
for comp_id in compare_list:
    ...
    compare_path = Path("PATH_TO_CALCS/ConvCheck/{comp_id}_4HSE/")
    base_data_path = Path(f"PATH_TO_DATABASE/{comp_id}/4HSE_HSEDOS/")
    ...
    fig.update_yaxes(title_text="E - Efermi (eV)", row=1, col=1)
    fig.update_yaxes(title_text="E - Efermi (eV)", row=1, col=2)
    ...
    fig.update_layout(
        ...
        legend=dict(x=1.02, y=1, font={'size':24}),
        width=1400,
        height=600,
