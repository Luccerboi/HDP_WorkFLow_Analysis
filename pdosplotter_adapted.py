...
def plot_pdos(
    ...
    *,
    ...
    ax.fill_between(
        x, 0, d["B1"][Spin.up], color=colors[0], alpha=0.1, label=comp_info["B1"]
    )
    ...
    if comp_info["B2"] == "Vac":
        ...
        ax.fill_between(
            x, 0, d["B2"][Spin.up], color=colors[1], alpha=0.1, label=comp_info["B2"]
        )
    ...
    ax.fill_between(
        x, 0, d["A"][Spin.up], color=colors[2], alpha=0.1, label=comp_info["A"]
    )
    ...
    ax.fill_between(
        x, 0, d["X"][Spin.up], color=colors[3], alpha=0.1, label=comp_info["X"]
    )
    ...
    for sp in range(1, len(axes)):
        ...
        for ii in range(len(bands)):
            ...
            ax.fill_between(
                ...
                alpha=0.4,
                hatch=marks[ii],
                label=(comp_info[s] + f"-({bands[ii]})"),
            )
            ax.fill_between(
                x, ddown[ii, :], color=colors[sp - 1], alpha=0.4, hatch=marks[ii]
            )
    ...
    fig.suptitle(
        f'{comp_info["A"]+ "2" +comp_info["B1"]+comp_info["B2"]+comp_info["X"] + "6"} projected DOS\n',
        fontsize=20,
        y=1.035,
    )
    if len(magmom) == 0:
...
def plot_tdos(
    ...
    *,
    ...
    ax.fill_between(
        x,
        0,
        d["B1"]["densities"][Spin.up],
        color=colors[0],
        alpha=0.1,
        label=str(element_map["B1"].element),
    )
    ax.fill_between(
        x, 0, d["B1"]["densities"][Spin.down] * (-1), color=colors[0], alpha=0.1
    )
    # B2
    if element_map["B2"] == None:
        ...
        ax.plot(
            x, d["B2"]["densities"][Spin.up], color=colors[1], alpha=0.8, linewidth=0.5
        )
        ax.plot(
            x,
            d["B2"]["densities"][Spin.down] * (-1),
            color=colors[1],
            alpha=0.8,
            linewidth=0.5,
        )
        ax.fill_between(
            x,
            0,
            d["B2"]["densities"][Spin.up],
            color=colors[1],
            alpha=0.1,
            label=str(element_map["B2"].element),
        )
        ax.fill_between(
            x, 0, d["B2"]["densities"][Spin.down] * (-1), color=colors[1], alpha=0.1
        )
    ...
    ax.fill_between(
        x,
        0,
        d["A"]["densities"][Spin.up],
        color=colors[2],
        alpha=0.1,
        label=str(element_map["A"].element),
    )
    ax.fill_between(
        x, 0, d["A"]["densities"][Spin.down] * (-1), color=colors[2], alpha=0.1
    )
    ...
    ax.fill_between(
        x,
        0,
        d["X"]["densities"][Spin.up],
        color=colors[3],
        alpha=0.1,
        label=str(element_map["X"].element),
    )
    ax.fill_between(
        x, 0, d["X"]["densities"][Spin.down] * (-1), color=colors[3], alpha=0.1
    )
    ...
    if element_map["B2"] != None:
        ...
        ax.set_title(
            f'{str(element_map["A"].element)+"2"+str(element_map["B1"].element)+str(element_map["B2"].element)+str(element_map["X"].element)+ "6"} projected DOS',
            fontsize=18,
            va="baseline",
            y=1.05,
        )
        ...
        ax.set_title(
            f'{str(element_map["A"].element)+"2"+ str(element_map["B1"].element)+ "Vac" +str(element_map["X"].element)+ "6"} projected DOS',
            fontsize=18,
            va="baseline",
            y=1.05,
        )
