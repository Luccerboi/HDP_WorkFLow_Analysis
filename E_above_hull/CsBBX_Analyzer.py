from CsBBCl_ht_new import ProcessLedger, compound
import pandas as pd
import numpy as np
import os
...
class GroupedAnalysis:
    ...
    def get_lobquality_data(self):
        ...
        # with ThreadPoolExecutor(max_workers=8) as executor, tqdm(total=len(self.analysis_list), desc="Processing compounds") as pbar:
        #     results = list(executor.map(process_comp, self.analysis_list))
        #     pbar.update(len(results))
        # # all_dicts = dict(results)
        # ii = 0
        # records = []
    ...
    def save_cba_all(self, save_dir: str | PathLike | None = None):
...
# 
    ...
    GroupedAnalysis.remove_duplicates(
        ...
        structuraldf=dstuc,
        basicdf=dbasic,
        to_sim_overview=Path('/home/lwalterb/hdp_project/HDP_WorkFlow_Analysis/ToSimulateComps.csv'),
